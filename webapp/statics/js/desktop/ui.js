define(function(require, exports) {
    ...
    var _bindHotKey = function() {
        ...
        $(document).keydown(function(e) {
            ...
            if (e.keyCode == cmmand || !e.ctrlKey)
                Global.ctrlKey = false;//win=ctrl
        });
    };
    var _ieCss = function() {
        if (!$.browser.msie && navigator.userAgent.indexOf("Firefox") < 0)
            return;
        var top = 10;
        var left = 10;
        var width = 80;
    ...
    this._getFileBox = function(list) {
        var html = '';
        if (list['type'] != 3 && list['icon'] != undefined) {
            var icon = list.icon;
            ...
            html = "<div class='file fileApp menuApp' data-app=" + code + " data-name='" + list.name + "' title='"
                    + LNG.name + ':' + list.name + "&#10;"
                    + LNG.modify_time + ':' + _getLocalTime(list.updatetime) + "'>";
            html += "<div class='ico' filetype='1' style='background-image:url(" + icon + ")'></div>";
            html += "<div id='' class='titleBox'><span class='title' title='" + LNG.double_click_rename + "'>" + list.name + "</span></div></div>";
        }else {
    ...
    var _getLocalTime = function(times) {     
    ...
    var _f5 = function(is_data_server, is_animate, callback) {
        if (is_data_server == undefined)
            is_data_server = true; //默认每次从服务器取数据
        if (is_animate == undefined)
            is_animate = false;	   //默认不用渐变动画
    ...
    return{
        ...
        init: function() {
            ...
            $("html").die('click').live('click', function(e) {
                rightMenu.hidden();
                if (Global.isIE && Global.isDragSelect)
                    return;
            });
